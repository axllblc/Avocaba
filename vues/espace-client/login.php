...
if ( !empty($_POST['email']) and !empty($_POST['motdepasse'])) {
  ...
  if($client AND $client != "wrong password"){
    //Si l'email est présente et que le mot de passe est bon, on met en session avec IdClient et on se redige vers l'espace client
    session_start();
    $_SESSION=$client;
    header('location: account.php');
  }
  else{
    // On défini un message qui indique que les informations de connexion ne sont pas correctes
  ...
  <main class="connexion">
    <div id="connexion_formulaire">
      <form class="connexion_connecter" action="login.php" method="POST">
        ...
        <label for="email" value="<?php if (isset($_POST['email'])) echo $_POST['email']; ?>">Adresse email</label><br>
        <input type="text" size="40" name="email" id="email" pattern="^[a-zA-Z1-9-\.]+@[a-zA-Z1-9-]+\.[a-zA-Z]{2,6}$" required><br>
        ...
        <span id="connexion_bouton_inscription"><a style="text-decoration:none" href="signin.php" title="Cliquez ici pour créer votre compte">Créer un compte</a></span>
      </form>
    </div>
  </main>
</body>
